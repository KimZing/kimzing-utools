...
module.exports = {
    getMacHistory() {
        ...
        let localHistorys = JSON.parse(fs.readFileSync(config.get().historyFile.mac).toString());
        localHistorys.forEach((item) => {
            let ide = getIDEInfo(item)
            let history = {title: item.name, description: item.path, icon: ide.icon, exec: ide.macExec};
